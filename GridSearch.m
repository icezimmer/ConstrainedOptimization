function [x_min, best, grid_search] = GridSearch(Q, q, P, x_start, eps, max_steps, eps_ls, left, right, delta)
%{
...
for i = 1:length(line_search_methods)
    for j = 1:length(beta_values)
        [x_min, f_min, elapsed_time, num_steps, converging, error] = FrankWolfe(Q, q, P, x_start, eps, max_steps, eps_ls, line_search_methods(i), beta_values(j));
